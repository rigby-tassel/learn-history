...
export default function AchievementBadge({ achievement, size = 'md', isNew }: AchievementBadgeProps) {
  ...
  return (
    <div className={cn(
      ...
      locked ? 'opacity-40 grayscale' : 'bg-white shadow-sm border border-slate-100',
      size === 'sm' && 'p-2',
    )}>
      {isNew && (
      ...
      <span className={cn('text-xs font-bold text-slate-800 text-center leading-tight', size === 'sm' && 'text-[10px]')}>
        {achievement.name}
      </span>
      {size === 'md' && (
